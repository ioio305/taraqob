// اختبار محسّن: وصفة خاصة لكل ورقة + حذف الخاسرين + فلتر اضطراب السوق
// الاستخدام: node scripts/backtest-v2.mjs stocks|funds
...
const COST = 0.0008
const VOL_SPIKE = 1.4      // تجاهل الإشارة إذا تجاوزت الحركة اليومية 1.4 ضعف معدلها
const TREND_TOLERANCE = 0.03 // سماحية 3% حول متوسط 200 يوم
...
// حساب متوسط حركة السوق لكل يوم مرة واحدة (للفلتر)
...
function simulate(symbol, bars, candidate, atrs, applyFilters) {
  ...
  for (let i = 205; i < bars.length - candidate.hold - 1; i++) {
    ...
    if (applyFilters) {
      ...
      if (avgVol && unit > avgVol * VOL_SPIKE) continue              // سوق مضطرب: لا توصية
      if (ma200) {
...
// العلاج 1+2: لكل ورقة نختار أفضل وصفة على فترة التدريب فقط، ونحذف من لا يثبت جدارته
const MODE = process.argv[3] === 'simple' ? 'simple' : 'persymbol'
const plan = new Map()
const dropped = []

if (MODE === 'simple') {
  // نهج متحفظ: وصفة واحدة للجميع تُختار على التدريب، ثم حذف الخاسرين فقط
  const ranked = CANDIDATES.map(candidate => {
    const trades = selectDailyBest(SYMBOLS.flatMap(s => simulate(s, datasets.get(s), candidate, atrMaps.get(s), true))).filter(t => t.date < SPLIT)
    return { candidate, m: metrics(trades) }
  }).sort((a, b) => b.m.expectancy - a.m.expectancy)
  const chosen = ranked[0].candidate
  for (const symbol of SYMBOLS) {
    const trainingTrades = simulate(symbol, datasets.get(symbol), chosen, atrMaps.get(symbol), true).filter(t => t.date < SPLIT)
    const m = metrics(trainingTrades)
    if (m.trades >= 15 && m.expectancy > 0) plan.set(symbol, { candidate: chosen, metrics: m })
  ...
  for (const symbol of SYMBOLS) {
    ...
    for (const candidate of CANDIDATES) {
      ...
      const m = metrics(trainingTrades)
      if (m.trades >= MIN_TRAIN_TRADES && m.expectancy > 0 && (!best || m.expectancy > best.metrics.expectancy)) {
        best = { candidate, metrics: m }
      }
    }
...
// المحاكاة النهائية بالخطة الجديدة
const allTrades = []
...
const perSymbol = [...plan.keys()].map(symbol => ({
  symbol,
  method: plan.get(symbol).candidate.name,
...
const pass = validation.trades >= 150 && validation.expectancy >= 0.12 && validation.profitFactor >= 1.25 &&
  validation.maxDrawdown <= 15 && training.expectancy > 0 && positiveCount >= Math.ceil(perSymbol.length * 0.7)
...
const report = `# تقرير الاختبار بعد خطة العلاج — توصيات ${platformLabel}
...
2. حذف الأوراق التي لم تثبت جدارتها في التدريب: **${dropped.join('، ') || 'لا يوجد'}**
3. فلتر اضطراب السوق: لا توصية عند ارتفاع حدة الحركة عن معدلها، ولا توصية عكس اتجاه السوق العام.

## النتيجة

**${pass ? 'اجتاز بوابة المعايرة ✅' : 'لم يجتز بوابة المعايرة بعد'}**
...
await writeFile(new URL(`../docs/${PLATFORM}-backtest-v2-report.md`, import.meta.url), report, 'utf8')
console.log(JSON.stringify({
  pass,
  qualified: [...plan.keys()],
  dropped,
  perSymbolMethods: Object.fromEntries([...plan.entries()].map(([s, e]) => [s, e.candidate.name])),
