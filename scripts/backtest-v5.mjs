// نسخة المعايرة المتجددة: كل سنة يعيد النظام اختيار أفضل وصفة لكل ورقة من آخر 3 سنوات فقط
// الاستخدام: node scripts/backtest-v5.mjs stocks
...
function simulate(symbol, bars, candidate, atrs, dateFrom, dateTo) {
  const trades = []
  for (let i = 205; i < bars.length - candidate.hold - 1; i++) {
    const date = bars[i + 1].date
    if (dateFrom && date < dateFrom) continue
    if (dateTo && date >= dateTo) break
    ...
    for (let j = i + 1; j <= i + candidate.hold; j++) {
      ...
      if (hitStop) { exit = stop; break }
      if (hitTarget) { exit = target; break }
    }
    const rawR = side * (exit - entry) / unit
    const costR = entry * COST / unit
    trades.push({ symbol, date, side, strength: signal.strength, r: rawR - costR })
    i += candidate.hold - 1
  }
  return trades
}
...
// المعايرة المتجددة: لكل سنة تحقق، نختار الوصفة من السنوات الثلاث السابقة فقط
const YEARS = [2023, 2024, 2025, 2026]
const yearlyRows = []
const walkTrades = []
const choicesLog = []
for (const year of YEARS) {
  const windowStart = `${year - LOOKBACK_YEARS}-01-01`
  ...
  for (const symbol of SYMBOLS) {
    ...
    for (const candidate of CANDIDATES) {
      const histTrades = simulate(symbol, bars, candidate, atrs, windowStart, yearStart)
      const m = metrics(histTrades)
      if (m.trades < MIN_TRAIN_TRADES || m.expectancy <= 0) continue
      if (!best || m.expectancy > best.m.expectancy) best = { candidate, m }
    }
    if (!best) { choicesLog.push(`${year} ${symbol}: مستبعد`); continue }
    choicesLog.push(`${year} ${symbol}: ${best.candidate.name}`)
    yearTrades.push(...simulate(symbol, bars, best.candidate, atrs, yearStart, yearEnd))
  }
  const daily = selectDailyBest(yearTrades)
  walkTrades.push(...daily)
  yearlyRows.push({ year, ...metrics(daily) })
}

const validation = metrics(selectDailyBest(walkTrades))
const perSymbol = SYMBOLS.map(symbol => ({
  symbol, ...metrics(walkTrades.filter(t => t.symbol === symbol)),
})).filter(row => row.trades > 0)
const positiveCount = perSymbol.filter(x => x.expectancy > 0).length

const platformLabel = PLATFORM === 'stocks' ? 'الشركات' : 'الصناديق'
const yearTable = yearlyRows.map(r => `| ${r.year} | ${r.trades} | ${fmt(r.winRate, 1)}% | ${fmt(r.expectancy)}R | ${fmt(r.profitFactor)} | ${fmt(r.maxDrawdown)}R |`).join('\n')
const symTable = perSymbol.map(row => `| ${row.symbol} | ${row.trades} | ${fmt(row.winRate, 1)}% | ${fmt(row.expectancy)}R | ${fmt(row.profitFactor)} | ${fmt(row.maxDrawdown)}R |`).join('\n')
const report = `# تقرير المعايرة المتجددة — توصيات ${platformLabel}

تاريخ التشغيل: ${new Date().toISOString()}

الفكرة: في بداية كل سنة، يعيد النظام اختيار أفضل وصفة لكل ورقة اعتمادًا على آخر ${LOOKBACK_YEARS} سنوات فقط، ويستبعد من لا يثبت جدارته. اختبار صارم: كل قرار مبني على الماضي فقط.

## النتيجة الإجمالية (2023 → الآن)

| الصفقات | النجاح | متوسط الصفقة | معامل الربح | أكبر تراجع |
|---:|---:|---:|---:|---:|
| ${validation.trades} | ${fmt(validation.winRate, 1)}% | ${fmt(validation.expectancy)}R | ${fmt(validation.profitFactor)} | ${fmt(validation.maxDrawdown)}R |
...
الأوراق الموجبة: ${positiveCount} من ${perSymbol.length}

## حد مهم

الاختبار يقيس اتجاه الورقة فقط، ولا يثبت ربح عقد الخيار نفسه.
`
await writeFile(new URL(`../docs/${PLATFORM}-backtest-v5-report.md`, import.meta.url), report, 'utf8')
console.log(JSON.stringify({ validation, yearly: yearlyRows, positiveSymbols: positiveCount, totalSymbols: perSymbol.length }, null, 2))
