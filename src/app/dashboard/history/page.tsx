...
export default function AnalysisDetailPage() {
  ...
  if (!a) return (
    <div className="p-6 text-center" dir="rtl">
      ...
      <Link href="/dashboard/history" className="btn-primary btn-sm mt-4 mx-auto block w-fit">العودة للسجل</Link>
    </div>
  )

  const mid = a.mid ?? ((a.bid ?? 0) + (a.ask ?? 0)) / 2
  const score = a.composite_score ?? 0
  const liveSpx = live?.spx?.price ?? null
  const liveVix = live?.vix?.price ?? null
  const spxDiff = liveSpx && a.spx_price ? liveSpx - a.spx_price : null
  const spxDiffPct = spxDiff && a.spx_price ? Math.abs(spxDiff / a.spx_price * 100) : 0

  const validity = !liveSpx
    ? { label: 'السوق مغلق', color: 'bg-surface-100 text-surface-600', desc: 'البيانات الحية غير متاحة' }
    : spxDiffPct > 1.5
    ? { label: '⚠️ تغير كبير — أعد التحليل', color: 'bg-red-50 border border-red-200 text-red-700', desc: `SPX تحرك ${spxDiff! > 0 ? '+' : ''}${spxDiff!.toFixed(0)} نقطة منذ التحليل` }
    : spxDiffPct > 0.5
    ? { label: 'تغير ملحوظ', color: 'bg-amber-50 border border-amber-200 text-amber-700', desc: `SPX تحرك ${spxDiff! > 0 ? '+' : ''}${spxDiff!.toFixed(0)} نقطة` }
    : { label: '✅ التحليل لا يزال صالحاً', color: 'bg-emerald-50 border border-emerald-200 text-emerald-700', desc: 'SPX لم يتغير كثيراً' }
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto animate-fade-in" dir="rtl">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-bold text-navy-900">
            {a.ticker && a.ticker !== 'SPX' ? a.ticker : 'SPX'} {a.contract_type === 'put' ? '▼ Put' : '▲ Call'} {a.strike}
          </h1>
          <p className="text-xs text-surface-400">{new Date(a.created_at).toLocaleString('ar-SA')}</p>
        </div>
        <Link href={`/dashboard/analyze?${reParams}`} className="btn-primary btn-sm">🔄 أعد التحليل</Link>
      </div>

      {/* صلاحية التحليل */}
      <div className={`rounded-2xl p-4 mb-4 ${validity.color}`}>
      ...
      <div className="card p-5 mb-4">
        <div className="text-xs font-semibold text-surface-400 mb-3">بطاقة التداول</div>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100">
            ...
            {mid > 0 && a.target1 && <div className="text-[10px] text-emerald-500 mt-0.5">+{((a.target1/mid-1)*100).toFixed(0)}% ربح</div>}
            {a.target2 && <div className="text-[10px] text-emerald-400 mt-0.5">جريء: ${a.target2.toFixed(2)}</div>}
          </div>
          <div className="bg-red-50 rounded-xl p-3 border border-red-100">
            ...
            {mid > 0 && a.stop_loss && <div className="text-[10px] text-red-400 mt-0.5">-{((1-a.stop_loss/mid)*100).toFixed(0)}%</div>}
          </div>
      ...
      {/* بيانات العقد */}
      <div className="card p-5 mb-4">
        <div className="text-xs font-semibold text-surface-400 mb-3">بيانات العقد</div>
        <div className="grid grid-cols-3 gap-2">
          {[
            { l:'Strike', v: String(a.strike??'--') },
            { l:'DTE',    v: a.dte ? `${a.dte} يوم` : '--' },
