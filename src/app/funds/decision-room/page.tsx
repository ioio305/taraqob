...
export default function FundsDecisionRoomPage() {
  ...
  const contractSymbol = opportunity?.best
    ? `${opportunity.symbol}${opportunity.best.expiration.replaceAll('-', '').slice(2)}${opportunity.best.type === 'call' ? 'C' : 'P'}${Math.round(opportunity.best.strike * 1_000).toString().padStart(8, '0')}`
    : ''
  const quoteSymbols = [
  ...
  const contractMid = quotes[contractSymbol]?.mid ?? quotes[contractSymbol]?.price ?? opportunity?.best?.mid
      ...
      <div className="mx-auto max-w-5xl space-y-4 p-4 md:p-7">
        <section className="grid gap-4 lg:grid-cols-[1.3fr_.7fr]">
          <div className="rounded-3xl border border-white/[.06] bg-[#0B1B15] p-6">
            ...
            {opportunity?.best ? <div className="mt-6 grid grid-cols-2 gap-2 md:grid-cols-4"><Metric label="العقد" value={`${opportunity.best.type.toUpperCase()} ${opportunity.best.strike}`} /><Metric label="القوة" value={`${opportunity.best.score}/100`} /><Metric label="السعر" value={`$${contractMid?.toFixed(2)}`} /><Metric label="الاحتمال" value={`${opportunity.best.probItmPct}%`} /></div> : null}
          </div>
